...
head(CCLE_MUT_CNA_AMP_DEL_binary_Revealer)
```
<br>
...
```{r}
heatmap(as.matrix(CCLE_MUT_CNA_AMP_DEL_binary_Revealer[1:1000, 1:1000]))
```
...
(tissues_occurrencies <- sort(tissues_occurrencies, decreasing = TRUE))
```
